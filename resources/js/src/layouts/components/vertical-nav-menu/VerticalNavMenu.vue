        ...
        <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" :key="$vs.rtl">
          <template v-for="(item, index) in menuItemsUpdated">
            ...
            <template v-else-if="!item.header">
              ...
              <!-- Nav-Item -->
              <v-nav-menu-item
                v-if="!item.submenu && $acl.check(item.rule) && (!item.maxRule || (item.maxRule && $acl.not.check(item.maxRule)))"
                :key="`item-${index}`"
                :index="index"
              ...
              :featherIcon = "false"
              :cat         = "item.cat">
                <span class="truncate ml-2">{{ $t(item.i18n) || item.name }}</span>
              </v-nav-menu-item>

              <!-- Nav-Group -->
              <template v-else-if="item.submenu&& (!item.maxRule || (item.maxRule && $acl.not.check(item.maxRule)))">
                <v-nav-menu-group
                  :key="`group-${index}`"
          ...
          <template v-if="$acl.not.check('admin')">
            <v-nav-menu-item
                :to="('/stores/edit/'+ activeUserInfo.store_id)"
                :href="('/stores/edit/'+ activeUserInfo.store_id)"
                slug="setting">
          ...
          <!-- <template v-if="$acl.not.check('captain')">
            <v-nav-menu-item
              ...
              :slug="item.slug">
              <img :src="'/images/'+ categoriesIconsFolder +'/'+item.icon+'.svg'" class="h-6 mt-1">
              <span v-show="!verticalNavMenuItemsMin" class="truncate ml-4">{{ $t(item.label) || item.label }}</span>
            </v-nav-menu-item>
          </template> -->
          ...
          <!-- <li
            ...
            v-for  = "(item, index) in categories"
            :key   = "item.id"
            :class = "{'mr-2': !(categories.length === index+1)}">

            <div v-if="$acl.not.check('captain')" class="menu-link">
              <h-nav-menu-item
                class       = "relative py-4 cursor-pointer"
                :to         = "item.slug !== 'external' ? item.url : null"
                :href       = "item.slug === 'external' ? item.url : null">
                  <img :src="'/images/'+ categoriesIconsFolder +'/'+item.icon+'.svg'" class="h-6 mt-1 ml-0 mr-2">
                  <span class="truncate">{{ $t(item.label) || item.label }}</span>
              </h-nav-menu-item>
            </div>
...
import SidebarIcon from '@/layouts/components/Sidebar-icon.vue';
