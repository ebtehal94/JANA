<template>
  ...
  <div id="page-order-list">
    ...
    <vx-card ref="filterCard" :title="$t('Filters')" class="order-list-filters mb-8" actionButtons @refresh="resetColFilters" @remove="resetColFilters">
      <div class="vx-row">
...
export default {
  ...
  data() {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: this.$i18n.locale == 'en' ? 'Price' : 'الإجمالي',
          field: 'total_price',
          filter: true,
          // width: 130,
  ...
  created() {
    ...
    this.$store.dispatch("orderManagement/fetchUserOrders", user.id).catch(err => { console.error(err) })
  }
}
