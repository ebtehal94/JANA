                    ...
                    <div class="vx-row ">
                        <div class="vx-col w-full lg:w-1/3">
                            ...
                            <div class="bg-input">
                                <vs-input
                                    class="w-full text-base mt-2 "
                                    :placeholder="$t('NewSponsorId')"
                                    v-model="newSponsorId"
                                    v-validate="{
                                                required: true,
                                                regex: /^(1|2|7)[0-9]{9}$/
                                                }"
                                    name="new-sponsorId"/>
                                <span v-if="!errors.has('new-sponsorId') && newSponsorId">
                                    <icon name="confirm" class="icon left-icon"/>
                                </span>
                                <span v-else-if="errors.has('new-sponsorId')">
                                    <icon name="cross" class="icon left-icon"/>
                                </span>
                            </div>
                        </div>
  ...
  export default{
    ...
    methods: {
      save_data() {
        ...
        axios.post('https://MUQEEM-API-URL/api/v1/iqama/transfer')
          .then((response) => {
