        ...
        <template slot="no-body">
            <div class="vx-row item-details no-gutter">
                ...
                <div class="vx-col sm:w-1/4 w-full item-img-container flex items-center justify-center cursor-pointer" @click="navigate_to_detail_view">
                    <img :src="productsLink+ item.images[0]" :alt="item.title_en" class="grid-view-img p-4">
                </div>
                ...
                <div class="vx-col sm:w-1/2">
                    <div class="p-4 pt-6">
                        <slot name="item-meta">
                            <h6 class="item-name font-semibold mb-1 hover:text-primary cursor-pointer" @click="navigate_to_detail_view">{{ item.title_en }}</h6>
                            <p class="text-sm mb-4">In <span class="font-semibold cursor-pointer">{{ item.category }}</span></p>
                            <p class="item-description text-sm">{{ item.desc_en }}</p>
                        </slot>
                    </div>
                </div>
