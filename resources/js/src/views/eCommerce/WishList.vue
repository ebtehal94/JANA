...
<template>
  <div id="ecommerce-wishlist-demo">
                    ...
                    <template slot="action-buttons">
                        <div class="flex flex-wrap">
                            ...
                            <div
                                ...
                                <span class="text-sm font-semibold ml-2">{{ $i18n.locale == 'en' ? 'REMOVE' : 'حذف' }}</span>
                            </div>
                            ...
                            <div
                                ...
                                <span class="text-sm font-semibold ml-2" v-else>{{ $i18n.locale == 'en' ? 'MOVE TO CART' : 'نقل إلي السلة' }}</span>
                            </div>
                        </div>
                    </template>
                </item-grid-view>
        ...
        <vx-card title="You don't have any items in your wish list." v-else>
            <vs-button @click="$router.push('/').catch(() => {})">Browse Shop</vs-button>
        </vx-card>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        cartButtonClicked(item) {
            if (this.isInCart(item.objectID)) this.$router.push('/checkout').catch(() => {})
            else {
