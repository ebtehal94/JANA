    ...
    <div id="shop-page">
        <ais-instant-search
            ...
            <div class="algolia-header mb-4">
                <div v-if="!$route.params.filter && !$route.params.category" class="flex md:items-end items-center justify-between flex-wrap">
                            ...
                            <ais-search-box>
                              <div slot-scope="{ currentRefinement, isSearchStalled, refine }">
                                <div class="relative">
                                  ...
                                  <!-- SEARCH ICON -->
                                  <div slot="submit-icon" class="absolute top-0 right-0 py-4 px-6" @click="refine($event)" v-show="!currentRefinement">
            ...
            <div id="algolia-content-container" class="relative clearfix">
                <vs-sidebar v-if="!$route.params.filter && !$route.params.category"
                    ...
                    <div class="p-6 filter-container">
                        ...
                        <ais-refinement-list attribute="filter_id">
                            <div slot-scope="{
                              items,
                              isFromSearch,
                              refine,
                                ...
                                <ul>
                                    ...
                                    <li id="ByFilter" v-if="filter != null" @click="refine(filter.id)"></li>
                                    <li v-for="item in items" :key="item.value" class="mb-2 flex items-center justify-between" @click="refine(item.value)">
                                        <vs-checkbox v-if="$i18n.locale == 'en'" v-model="item.isRefined" class="ml-0">{{ getFilterFromID(item.value).title_en }}</vs-checkbox>
                                        <vs-checkbox v-else v-model="item.isRefined" class="ml-0">{{ getFilterFromID(item.value).title_ar }}</vs-checkbox>
                        ...
                        <ais-clear-refinements class="flex justify-center">
                            <vs-button class="w-full" slot-scope="{ canRefine, refine }" @click.prevent="refine" :disabled="!canRefine">ازالة التصنيفات</vs-button>
                        </ais-clear-refinements>
                    </div>
                </vs-sidebar>

                <!-- RIGHT COL -->
                <div :class="{'sidebar-spacer-with-margin': clickNotClose && !$route.params.filter && !$route.params.category }">
                                    ...
                                    <item-grid-view class="" v-for="item in items" :key="item.objectID" :item="item">
                                      ...
                                      <template slot="action-buttons">
                                            <div class="md:flex md:flex-wrap justify-center">
                                                <!-- ADD-TO/VIEW-IN CART -->
                                                <vs-button v-if="item.details[shortCode].stock > 0"
                                                    class="item-view-secondary-action-btn bg-primary text-white"
                                                    icon-pack="feather"
                                                    size="small"
                                                ...
                                                <div class="flex flex-wrap">
                                                <!--  ADD TO WISH LIST -->
                                                  <feather-icon @click="toggleItemInWishList(item)" class="p-2 cursor-pointer" icon="HeartIcon" :svgClasses="[{'text-danger fill-current' : isInWishList(item.objectID)}, 'h-6 w-6']" />
                                                <!--  Share -->
                                                  <feather-icon icon="Share2Icon" class="p-2 cursor-pointer" :svgClasses="['h-6 w-6']" />
                                                </div>
                                            </div>
                                        </template>
                                      </item-grid-view>
...
export default {
  ...
  methods: {
    getFilterFromID(filter_id){
      return (this.filters.find((filter) => filter.id == filter_id)) ? this.filters.find((filter) => filter.id == filter_id) : this.$t('NA')
    },
    fetch_filters_data(){
      axios.get("/api/categories/filter/list")
      .then(res => {
        this.$store.dispatch('eCommerce/updateFiltersList', res.data)
        // this.filters = res.data
        this.filter = this.filters.filter(f => f.id == this.$route.params.filter)[0]
      })
  ...
  mounted () {
    setTimeout(() => {
      document.addEventListener('DOMContentLoaded', function () {
        const elem = document.getElementById('ByFilter')
        elem.click()
      })
    }, 50)
  },
...
<style lang="scss">
.vs-icon.notranslate.icon-scale.vs-button--icon.feather.icon-shopping-cart.null {
  z-index: 1 !important;
}
