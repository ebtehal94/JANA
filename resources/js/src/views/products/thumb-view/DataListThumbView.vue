    ...
    <vs-table ref="table" pagination :max-items="itemsPerPage" search :data="products">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ACTION - DROPDOWN -->
          <div class="p-4 mr-4 mb-4 shadow-drop rounded-lg d-theme-dark-bg cursor-pointer flex items-center justify-center text-lg font-medium w-32" @click="listView">
            <span class="mr-2">{{ $t('List') }}</span>
      ...
      <template slot="thead">
        <vs-th>{{ $i18n.locale == 'en' ? 'Image' : 'صورة المنتج' }}</vs-th>
        ...
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="img-container">
              <img v-if="tr.images.length > 0" :src="productsLink+tr.images[0].link" class="product-img" />
            </vs-td>
            ...
            <vs-td>
              <p v-if="tr.category" class="product-category">{{ $i18n.locale == 'en' ? (tr.category.title_en || 'N/A') : (tr.category.title_en || 'N/A') }}</p>
            </vs-td>
...
export default {
  ...
  methods: {
    listView(){
      this.$router.push({path: '/products/list-view'})
    },
    addNewData() {
